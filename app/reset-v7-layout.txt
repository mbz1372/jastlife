import "./globals.css";

export const metadata = {
  metadataBase: new URL("https://jastlife.vercel.app"),
  title: "JASTLIFE | Curated Outdoor Goods",
  description: "JASTLIFE curates camping, cycling, travel and outdoor tech for lighter movement and better time outside.",
  keywords: ["JASTLIFE", "outdoor gear", "camping", "cycling", "travel gear", "outdoor tech"],
  openGraph: {
    title: "JASTLIFE | Outside, made simpler.",
    description: "Curated outdoor goods for people who move lighter and live outside better.",
    type: "website",
    images: ["/hero.png"],
  },
  twitter: {
    card: "summary_large_image",
    title: "JASTLIFE | Outside, made simpler.",
    description: "Curated outdoor goods for people who move lighter and live outside better.",
    images: ["/hero.png"],
  },
};

export const viewport = {
  themeColor: "#f5f3ee",
  colorScheme: "light",
};

export default function RootLayout({ children }) {
  return (
    <html lang="fa" dir="rtl">
      <body>{children}</body>
    </html>
  );
}
